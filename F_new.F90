...
do i=1,m
    do j=1,n
        ...
        Radi_coeff(i,j)=(Ma*cos_thita(i,j)+dsqrt(1-Ma**2*sin_thita(i,j)**2));
 ...
 do i=1,3
    do j=1,n
        k_rou(i,j)=Radi_coeff(i,j)*(cos_thita(i,j)*Deriv_E1(i,j)+sin_thita(i,j)*&
				&Deriv_F1(i,j)-rou(i,j)/(2.0d0*dsqrt(x_coor(i)**2+y_coor(j)**2)));
        k_u(i,j)=Radi_coeff(i,j)*(cos_thita(i,j)*Deriv_E2(i,j)+sin_thita(i,j)*&
				&Deriv_F2(i,j)-u(i,j)/(2.0d0*dsqrt(x_coor(i)**2+y_coor(j)**2)));
        k_v(i,j)=Radi_coeff(i,j)*(cos_thita(i,j)*Deriv_E3(i,j)+sin_thita(i,j)*&
				&Deriv_F3(i,j)-v(i,j)/(2.0d0*dsqrt(x_coor(i)**2+y_coor(j)**2)));
        k_p(i,j)=Radi_coeff(i,j)*(cos_thita(i,j)*Deriv_E4(i,j)+sin_thita(i,j)*&
				&Deriv_F4(i,j)-p(i,j)/(2.0d0*dsqrt(x_coor(i)**2+y_coor(j)**2)));
...
!!
 ...
 !do i=4,m
    ...
    do j=(n-2),n
        !k_rou(i,j)=Radi p_37_coeff_cos(i,j)*Deriv_E1(i,j)+Radi_coeff_sin(i,j)*Deriv_F1(i,j);
        !k_u(i,j)=Radi_coeff_cos(i,j)*Deriv_E2(i,j)+Radi_coeff_sin(i,j)*Deriv_F2(i,j);
        !k_v(i,j)=Radi_coeff_cos(i,j)*Deriv_E3(i,j)+Radi_coeff_sin(i,j)*Deriv_F3(i,j);
        !k_p(i,j)=Radi_coeff_cos(i,j)*Deriv_E4(i,j)+Radi_coeff_sin(i,j)*Deriv_F4(i,j);
!
...
do i=(m-2),m
 !do j=4,n-3
    do j=1,n
        ...
        k_p(i,j)=Radi_coeff(i,j)*cos_thita(i,j)*Deriv_E4(i,j)+Radi_coeff(i,j)*sin_thita(i,j)*Deriv_F4(i,j)&
            &-p(i,j)*Radi_coeff(i,j)/(2.0d0*dsqrt(x_coor(i)**2+y_coor(j)**2));
